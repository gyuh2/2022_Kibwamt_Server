...
//========================================================
#include "WiFiEsp.h"
char ssid[] = "SRC";   // your network SSID (name)
char pass[] = "!src@5146#";  // your network password
...
IPAddress hostIp(192, 168, 118, 207);
...
void control(int c){
  switch(c){
    ...
    default:
      ...
      delay(1000); // 10초 뒤에 서버와 연결을 끊고 재연결 시도
      client.stop();
  }
...
void remoteControl() { //원격제어 참조 및 제어 코드
  if(client.connect(hostIp, SERVER_PORT)){
    ...
    client.flush();
    client.stop();  
    delay(1000); //30초 뒤 서버와 연결을 끊고 재연결 시도
  }else {
...
void loop() { 
   ...
   if(timeout == 1){ //처음
     ...
     Serial.println("1 connect start");
     connectToServer(); //데이터 전송
     Serial.println("1 Data send and get");
     fromServer(); //데이터 수신
     ...
     printLCD(0, 0, sData);
     memset(sData, 0x00, 64);
     sprintf(sData, "dust: %d", (int)dustDensity);
     printLCD(0, 1, sData);
     ...
     delay(3000); // 3초 뒤
      ...
      if(humidity != h || temperature != t || dustDensity != d){ 
         ...
         connectToServer(); //데이터 전송
         Serial.println("Data send and get");
         fromServer(); //데이터 수신
